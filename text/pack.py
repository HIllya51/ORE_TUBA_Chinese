...
def findx(s, string, start=0):
    for i in range(start, len(string)):
        if string[i] == s and string[i + 9] == s:
            return i
    return -1
...
for current in ["CHS"]:  # ,'CHT']:
    for f in os.listdir(f"./{current}/translatedok"):
        ...
        for i in range(len(translines)):
            ...
            if line and line[0] == "○":
                ...
                if (
                    ...
                    or translines[i].startswith("○0000470D○")
                    or translines[i].startswith("○0000F23E○")
                    or "$&" in translines[i]
                ) and translines[i][-1] != "$":
                    translines[i] += "$"
                if translines[i].startswith("○00002385○") and translines[i][-1] != "。":
                    translines[i] += "。"
                ...
                if (
                    ...
                    translines[i] = translines[i].replace(" ", "")
                if (
                    translines[i].startswith("○00009F49○")
                    or translines[i].startswith("○00006AFE○")
                    or translines[i].startswith("○00005EDE○")
                    or translines[i].startswith("○00009E7A○")
                    or translines[i].startswith("○00010BDC○　（")
                    or translines[i].startswith("○00016FA2○")
                    or translines[i].startswith("○00006021○　ア")
                    or translines[i].startswith("○00010A88○")
                    or translines[i].startswith("○00004A74○")
                    or translines[i].startswith("○00005549○")
                    or translines[i].startswith("○00005EDE○")
                ):
                    translines[i] = translines[i].replace("○　", "○")
        transed = "\n".join(translines)
        ...
        with open(
            f"./{current}/merge/" + f.replace(" ", ""), "w", encoding="utf8"
        ) as _:
            # for k in origin:
            #     _.write(k)
            #     _.write('\n')
