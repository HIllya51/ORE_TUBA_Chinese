...
std::vector<std::wstring>textsplit(FontInfo mFontInfo,int w, ::Graphics* const graphics,const wchar_t* mText) {
	std::wstring text = mText;
	std::vector<std::wstring> lines;
	std::wstring line;
	Font font(mFontInfo.FontName, mFontInfo.FontSize);
	...
	for (const auto& c : text)
	{
		...
		{
			graphics->MeasureString(line.c_str(), line.size(), &font, PointF((float)0, (float)0), &rclayout);
			if (rclayout.Height > w) {
...
std::pair<RectF, Bitmap*> OverlayLyric::drawaline(::Graphics* const graphics, const SIZE& size,const wchar_t* text,int idx) {
	RECT rect;
   ...
   // ������ʱλͼ
    auto bitmap=new Bitmap(boardWidth, boardHeight, PixelFormat32bppARGB);
   ::Graphics bitmapGraphics(bitmap);

   ::StringFormat format;
   format.SetAlignment(::StringAlignmentNear);
   ::FontFamily fontFamily(mFontInfo.FontName);
    ::Font font(&fontFamily, mFontInfo.FontSize, FontStyleBold, UnitPixel); 
   ::GraphicsPath path;
   ::RectF rclayout(0.f, 0.f, REAL(boardWidth), REAL(boardHeight));
	...
	::SolidBrush brushB(0xFF000000);
	for (int i = 0; i < layerNum; i++) {
		rclayout.X = offset[i];
		rclayout.Y = 5+offset[i];  
		bitmapGraphics.DrawString(text,-1,&font,rclayout,&format,&brushB);
	}
   rclayout.X = 0.f;
   rclayout.Y = 5+0.f;
   path.AddString(text, -1, &fontFamily, ::FontStyleBold, mFontInfo.FontSize, rclayout, &format); 
  	SolidBrush brushW(Color(255, 255, 255, 255)); 
  
  //bitmapGraphics.FillPath(&brush, &path);
 
  
	bitmapGraphics.DrawString(text,-1,&font,rclayout,&format,&brushW);
...
//    bitmapGraphics.Flush();
   ...
   //graphics->DrawImage(clone, 0.f, rcBound.Height* idx);
   //delete clone; // �ͷ��ڴ�
   return std::make_pair(rcBound , bitmap);
...
BOOL OverlayLyric::DrawText(::Graphics* const graphics, const SIZE& size)
{
	...
	auto lines = textsplit(mFontInfo,boardWidth,graphics, mText);
	 

	std::vector< std::pair<::RectF, Bitmap*>>save;
	float maxh = 0;
	for (size_t i = 0; i < lines.size(); i++)
	{
		auto lineimg=drawaline(graphics, size, lines[i].c_str(),i);
		save.emplace_back(lineimg);
		if (lineimg.first.Height > maxh) {
			maxh = lineimg.first.Height;
		}
	}
	float ystart = (size.cy - maxh * save.size()) / 2;
	for (int i = 0; i < save.size(); i++) {
		 
		graphics->DrawImage(save[i].second, size.cx / 2 - save[i].first.Width / 2, ystart+ maxh * i);
		delete save[i].second; // �ͷ��ڴ�
	}
...
LRESULT OverlayLyric::HandleMessage(UINT message, WPARAM wParam, LPARAM lParam) {
	...
	{
		/// | WS_EX_TRANSPARENT
		SetWindowLong(hWnd, GWL_EXSTYLE, WS_EX_LAYERED | WS_EX_TOOLWINDOW | WS_EX_TOPMOST);  // ���ô�����չ��ʽ
	...
	// 
	case WM_TIMER:
		if (wParam == IDT_MOUSETRAP) {
			POINT udtPoint;
			GetCursorPos(&udtPoint);	//��ȡ�������
			HWND lonCurHWND = WindowFromPoint(udtPoint);	//��ȡ����ϵľ��
			if (lonCurHWND == GetHandle()) {
				if (mShowBackground == FALSE) {
					mShowBackground = TRUE;
					if (mconnecthlrc) {
						LyricShow(mconnecthlrc);
						LyricUpdate(mconnecthlrc);
					}
				...
				if (mShowBackground == TRUE) {
					mShowBackground = FALSE;
					if (mconnecthlrc) {
						LyricHide(mconnecthlrc);
						LyricUpdate(mconnecthlrc);
					} 
					Update();
				}
